...
const FeaturedSection = ({ songs, isLoading }: FeaturedSectionProps) => {
  const { currentSong, togglePlay, setCurrentSong } = usePlayer()
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
      {songs?.map((featuredSong) => (
        <div
          ...
          onClick={() => handleCardClick(featuredSong)}
          className="flex items-center bg-zinc-800/50 hover:bg-zinc-700/50 rounded-md overflow-hidden cursor-pointer group relative transition-colors"
        >
          <img
